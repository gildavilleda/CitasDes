...
//Procedimiento para agregar o modificar
        try {
            ...
            $stmt = $Db->Sentencia("EXEC seguridad.sp_AgoModifpermisosRoles ?, ?, ?, ?");

            $stmt->bindParam(1, $_REQUEST{'SegNumPermisos'});
            $stmt->bindParam(2, $_REQUEST{'SegNumPermisos'});
...
//Procedimiento para Eliminar
        try {
            ...
            $stmt = $Db->Sentencia("EXEC seguridad.sp_ElimPermisos ?, ?, ?, ?");

            $stmt->bindParam(1, $_REQUEST{'SegNumRol'});
            $stmt->bindParam(2, $_REQUEST{'SegNumPermisos'});
            $stmt->bindParam(3, $codeMensaje, PDO::PARAM_INT | PDO::PARAM_INPUT_OUTPUT, 50);
            $stmt->bindParam(4, $strMensaje, PDO::PARAM_STR | PDO::PARAM_INPUT_OUTPUT, 50);
            $stmt->execute();
...
//Procedimiento para Mostrar
        ...
        $data = $Db->GetData("SELECT * FROM seguridad.RelPerRol", $_SESSION['dbUser'], $_SESSION['dbPass']);
